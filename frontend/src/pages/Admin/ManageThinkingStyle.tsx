...
import { ThinkingStyle, CreateThinkingStylePayload, UpdateThinkingStylePayload } from '@/services/api/thinkingStylesAdmin';
...
const ThinkingStylesManagement = () => {
  ...
  const [modalType, setModalType] = useState<'create' | 'edit' | 'view' | 'bulk'>('create');
  ...
  const openModal = (type: 'create' | 'edit' | 'view' | 'bulk', style: ThinkingStyle | null = null) => {
    setModalType(type);
    ...
    if (type === 'edit' && style) {
      ...
      setBulkUpdates(thinkingStyles.map(style => ({
        id: style.id,
        digit: style.digit,
        type: style.type,
        code: style.code,
  ...
  const handleBulkUpdate = async () => {
    ...
    bulkUpdateMutation.mutate({ updates: validUpdates as UpdateThinkingStylePayload[] }, {
      onSuccess: () => {
        closeModal();
      }
    });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Thinking Styles Management</h1>
              <p className="text-gray-600">Kelola thinking styles untuk sistem assessment</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{statistics?.data?.summary?.total || 0}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{statistics?.data?.summary?.active || 0}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Inactive Styles</p>
                <p className="text-2xl font-bold text-red-600">{statistics?.data?.summary?.inactive || 0}</p>
              </div>
      ...
      {/* Add/Edit Modal */}
      <Dialog open={showModal && (modalType === 'create' || modalType === 'edit')} onOpenChange={closeModal}>
        <DialogContent className="max-w-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Digit *</label>
                <Input
                  type="number"
                  min="1"
                  max="9"
                  value={formData.digit}
                  onChange={(e) => setFormData(prev => ({...prev, digit: Number(e.target.value)}))}
                  required
                />
              </div>
              <div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="mt-1">{getDigitBadge(currentStyle.digit)}</div>
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <p className="text-gray-900 mt-1 text-lg font-semibold">{currentStyle.type}</p>
                </div>
                <div className="col-span-2">
                  <span className="font-medium text-gray-500">Description:</span>
                  <p className="text-gray-900 mt-1">{currentStyle.description}</p>
                </div>
                <div className="col-span-2">
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <Badge variant={currentStyle.isActive ? 'default' : 'destructive'}>
                ...
                <div>
                  ...
                  <p className="text-gray-900 mt-1">{new Date(currentStyle.createdAt).toLocaleDateString('id-ID')}</p>
                </div>
              </div>
            </div>
          )}
                ...
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-gray-500">({update.code})</span>
                    </div>
                    <Button
                      ...
                      onClick={() => {
                        ...
                        updatedBulk[index].isActive = !updatedBulk[index].isActive;
                        setBulkUpdates(updatedBulk);
                      }}
                    >
                      {update.isActive ? <ToggleRight className="w-4 h-4 mr-2" /> : <ToggleLeft className="w-4 h-4 mr-2" />}
